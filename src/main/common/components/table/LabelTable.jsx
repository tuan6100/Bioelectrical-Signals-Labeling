...
import {fetchUpdateAnnotation, fetchDeleteAnnotation, fetchCreateLabel, fetchShowErrorDialog, fetchGetAllLabels} from '../../api/index.js'
...
const LabelTable = ({ data, onDeleteRow, onLabelChange, channelId }) => {
  ...
  const prevRowsMapRef = React.useRef(new Map())
      ...
      if (selectedStillExists) {
        try {
          ...
          if (prevLength === data.length && changedCount === 1 && changedId != null) {
            if (selectedId === changedId) {
              setSelectedId(null)
              setTimeout(() => setSelectedId(changedId), 0)
            } else {
              setSelectedId(changedId)
            }
          }
      ...
      if (updatedId != null) {
        ...
        if (exists) {
          if (selectedId === updatedId) {
            setSelectedId(null)
            setTimeout(() => setSelectedId(updatedId), 0)
          } else {
      ...
      {(() => {
        ...
        <tbody>
          ...
          <tr>
              ...
              <td>
                  {/* Replace select with input + datalist for free text or existing label selection */}
                  <input
                      type="text"
                      list="label-options"
